...
class RidgeRegression(torch.nn.Module):
    ...
    def forward(self, x, subj_idx=None):
        if subj_idx is None:
            if isinstance(x, list):
                out = torch.cat([linear(x[i]) for i, linear in enumerate(self.linears)], dim=0)
        else:
            out = self.linears[subj_idx](x)
        return out

class BrainNetwork(nn.Module):
    def __init__(self, h=4096, in_dim=15724, out_dim=768, seq_len=2, n_blocks=4, drop=.15, clip_size=768, blurry_recon=False, clip_scale=1, interm_out=False):
        super().__init__()
        ...
        # Output linear layer
        if interm_out:
            self.interm_linear = nn.ModuleList([nn.Linear(h * seq_len, out_dim, bias=True) for _ in range(n_blocks - 1)])
            self.clip_proj_interm = nn.ModuleList([self.projector(clip_size, clip_size, h=clip_size) for _ in range(n_blocks - 1)])
        self.backbone_linear = nn.Linear(h * seq_len, out_dim, bias=True)
        self.clip_proj = nn.Linear(clip_size, clip_size)
        
        # Blurry reconstruction layers
    ...
    def forward_feat(self, x, weight=None):
        interm_feats = []
        
        # Mixer blocks
        residual1 = x
        residual2 = x.permute(0,2,1)
        for i, (block1, block2) in enumerate(zip(self.mixer_blocks1,self.mixer_blocks2)):
            ...
            if self.interm_out and i < 3: 
                # save intermediate features in the clip space
                interm_feats.append(self.clip_proj_interm[i](self.interm_linear[i](x.reshape(x.size(0), -1)).reshape(len(x), -1, self.clip_size)))
        return x, interm_feats

    def forward(self, x, weight=None):
        c,b = torch.Tensor([0.]), None
        x, interm_feats = self.forward_feat(x, weight)

        if weight is not None: x *= weight.unsqueeze(1)
        x = x.reshape(x.size(0), -1)

        # project to OpenCLIP ViT-bigG/14 image token space
        ...
        if self.interm_out:
            return backbone, c, b, interm_feats
        return backbone, c, b


class BrainNetworkUp(BrainNetwork):
    """Linear projection for the CLIP seq_len at every stage, followed by mixer blocks."""
    def __init__(self, h=4096, in_dim=15724, out_dim=768, seq_len=2, n_blocks=4, drop=.15, clip_size=768, blurry_recon=True, clip_scale=1, interm_out=False):
        super().__init__(h, in_dim, out_dim, seq_len, n_blocks, drop, clip_size, blurry_recon, clip_scale, interm_out)
        self.seq_len = seq_len
        self.h = h
        self.clip_size = clip_size
        self.blurry_recon = blurry_recon
        ...
        clip_seq_len = out_dim // clip_size
        clip_lens = [int((i + 1) * (clip_seq_len / n_blocks)) for i in range(n_blocks)]
        clip_lens = [1] + clip_lens
        self.upsample_linear = nn.ModuleList([nn.Linear(seq_len * clip_lens[i], clip_lens[i + 1], bias=True) for i in range(n_blocks)])
        
        self.mixer_blocks1 = nn.ModuleList([
            self.mixer_block1(h, drop) for _ in range(n_blocks)
        ])
        self.mixer_blocks2 = nn.ModuleList([
            self.mixer_block2(clip_lens[i], drop) for i in range(n_blocks)
        ])
   
    def forward(self, x):
        if self.interm_out: interm_feats = []
        # make empty tensors
        ...
        for i, (block1, block2) in enumerate(zip(self.mixer_blocks1,self.mixer_blocks2)):
            ...
            x = block2(x) + residual2
            x = self.upsample_linear[i](x)
            x = x.permute(0,2,1)

            residual1, residual2 = x, x.permute(0,2,1)
        ...
        # project to OpenCLIP ViT-bigG/14 image token space
        backbone = self.backbone_linear(x)
...
class BrainNetworkEarly(BrainNetwork):
    def __init__(self, h=4096, in_dim=15724, out_dim=768, seq_len=2, n_blocks=4, drop=.15, clip_size=768, blurry_recon=True, clip_scale=1, interm_out=False):
        super().__init__(h, in_dim, out_dim, seq_len, n_blocks, drop, clip_size, blurry_recon, clip_scale, interm_out)
        self.seq_len = seq_len
        self.clip_len = out_dim // clip_size
        self.h = h
        self.clip_size = clip_size
        self.blurry_recon = blurry_recon
        self.clip_scale = clip_scale
        ...
        self.mixer_blocks2 = nn.ModuleList([
            self.mixer_block2(self.clip_len, drop) for _ in range(n_blocks)
        ])
        
        self.backbone_linear = nn.Linear(h * seq_len, self.clip_len * h, bias=True)
        self.backbone_linear2 = nn.Linear(h, self.clip_size, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.backbone_linear(x).reshape(len(x), self.clip_len, -1)
        ...
        # project to OpenCLIP ViT-bigG/14 image token space
        backbone = self.backbone_linear2(x)
...
class BrainNetworkMulti(BrainNetwork):
    def __init__(self, h=4096, in_dim=15724, out_dim=768, seq_len=2, n_blocks=4, drop=.15, clip_size=768, blurry_recon=True, clip_scale=1, interm_out=False):
        super().__init__(h, in_dim, out_dim, seq_len, n_blocks, drop, clip_size, blurry_recon, clip_scale, interm_out)
        self.seq_len = seq_len
        ...
        self.mixer_blocks2 = nn.ModuleList([
            self.mixer_block2(seq_len, drop) for _ in range(n_blocks // 2)
        ])
        self.mixer_blocks2.extend(nn.ModuleList([self.mixer_block2(mid_clip_len, drop) for _ in range(n_blocks // 2)]))
        self.upsample_linear = nn.Linear(h * seq_len, h * mid_clip_len, bias=True)
        
        # Output linear layer
        if interm_out:
            self.interm_linear = nn.ModuleList([nn.Linear(h * seq_len, out_dim, bias=True) for _ in range(n_blocks - 1)])
            self.clip_proj_interm = nn.ModuleList([self.projector(clip_size, clip_size, h=clip_size) for _ in range(n_blocks - 1)])
        self.backbone_linear = nn.Linear(h * mid_clip_len, out_dim, bias=True)
    ...
    def forward(self, x):
        ...
        for i, (block1, block2) in enumerate(zip(self.mixer_blocks1,self.mixer_blocks2)):
            ...
            if i == self.n_blocks // 2 - 1:
                x = self.upsample_linear(x.flatten(1)).reshape(len(x), residual1.shape[-1], -1)
