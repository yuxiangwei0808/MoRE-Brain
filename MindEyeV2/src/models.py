...
class BrainNetwork(nn.Module):
    def __init__(self, h=4096, in_dim=15724, out_dim=768, seq_len=2, n_blocks=4, drop=.15, clip_size=768, blurry_recon=False, clip_scale=1):
        super().__init__()
...
class BrainDiffusionPrior(DiffusionPrior):
    ...
    def p_losses(self, image_embed, times, text_cond, noise = None):
        ...
        if pred.shape != target.shape:
            loss = nn.functional.mse_loss(pred.mean(1), target.mean(1))
        else:
...
class PriorNetwork(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if self.learned_query_mode == 'token':
            self.learned_query = nn.Parameter(torch.randn(num_tokens_target, dim))
        if self.learned_query_mode == 'pos_emb':
            scale = dim ** -0.5
            self.learned_query = nn.Parameter(torch.randn(num_tokens_target, dim) * scale)
        if self.learned_query_mode == 'all_pos_emb':
            scale = dim ** -0.5
            self.learned_query = nn.Parameter(torch.randn(num_tokens_target*2+1, dim) * scale)
        ...
        self.null_image_embed = nn.Parameter(torch.randn(num_tokens_target, dim))
    ...
    def forward(
        ...
        *,
        ...
        pred_image_embed = tokens[..., -self.num_tokens_target:, :]
