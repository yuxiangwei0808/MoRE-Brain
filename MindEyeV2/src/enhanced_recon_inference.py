...
os.makedirs("/home/users/ywei13/playground/BrainGen/checkpoints/MindEye2/evals",exist_ok=True)
os.makedirs(f"/home/users/ywei13/playground/BrainGen/checkpoints/MindEye2/evals/{model_name}",exist_ok=True)
...
all_images = torch.load(f"/home/users/ywei13/playground/BrainGen/checkpoints/MindEye2/evals/all_images.pt")
...
all_clipvoxels = torch.load(f"/home/users/ywei13/playground/BrainGen/checkpoints/MindEye2/evals/{model_name}/{model_name}_all_clipvoxels.pt")
all_blurryrecons = torch.load(f"/home/users/ywei13/playground/BrainGen/checkpoints/MindEye2/evals/{model_name}/{model_name}_all_blurryrecons.pt")
...
base_ckpt_path = '/home/users/ywei13/playground/BrainGen/data/NSD/zavychromaxl_v30.safetensors'
...
for img_idx in tqdm(range(len(all_recons))):
    with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float16), base_engine.ema_scope():
        ...
        # init_z = (torch.ones_like(z) + noise * append_dims(sigmas[-img2img_timepoint], z.ndim)) / torch.sqrt(1.0 + sigmas[0] ** 2.0)
        # sigmas = sigmas[-img2img_timepoint:].repeat(num_samples,1)

        init_z = z / torch.sqrt(1.0 + sigmas[0] ** 2.0)  # Simple noise scaling
        sigmas = sigmas.repeat(num_samples,1)  # Use full sigma schedule
...
model_name += '_randomZ'
if not os.path.isdir(f"/home/users/ywei13/playground/BrainGen/checkpoints/MindEye2/evals/{model_name}"):
    os.makedirs(f"/home/users/ywei13/playground/BrainGen/checkpoints/MindEye2/evals/{model_name}")
torch.save(all_enhancedrecons,f"/home/users/ywei13/playground/BrainGen/checkpoints/MindEye2/evals/{model_name}/{model_name}_all_enhancedrecons.pt")
