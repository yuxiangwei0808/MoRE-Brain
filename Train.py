...
# Add necessary paths for MindEyeV2 dependencies
...
def setup_distributed_training():
    """Configure distributed training with DeepSpeed stage 2 and CPU offloading."""
    # Run "accelerate config" in terminal first
...
def configure_batch_size(batch_size, num_devices):
    ...
    if utils.is_interactive():
        # Set batch size for interactive notebook mode
        global_batch_size = batch_size = 8
    else:
        if "GLOBAL_BATCH_SIZE" not in os.environ:
            os.environ["GLOBAL_BATCH_SIZE"] = '21'
...
global_batch_size, batch_size = configure_batch_size(16, NUM_DEVICES)  # Default batch size
...
logging.info(f"Distributed: {distributed}, Devices: {NUM_DEVICES}, "
             f"Local rank: {local_rank}, World size: {world_size}, Data type: {DATA_TYPE}")
...
def create_argument_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Brain-to-Image Model Training Configuration",
    ...
    data_group.add_argument(
        ...
        help="Path for storing misc. files downloaded from HuggingFace"
    )
    data_group.add_argument(
    ...
    model_group.add_argument(
        "--is_image", default=True, action=argparse.BooleanOptionalAction,
    ...
    training_group.add_argument(
        "--train_router_only", default=False, action=argparse.BooleanOptionalAction,
        help="Only train expert routers with frozen experts (for cross-subject); need to supply ckpt_path"
    )
    
    # Loss scales and coefficients
...
def configure_subjects_and_test_data():
    ...
    if multi_subject:
        ...
        test_subj = subj_list[0]  # Use first subject for validation in multi-subject training
    else:
        subj_list = [subj]
        test_subj = subj
    
    print(f"Subject list: {subj_list}, Number of sessions: {num_sessions}")
    
    # Configure test dataset based on subject and test set version
    if not new_test:  # Original test set from MindEye paper
    ...
    else:  # Larger test set from full dataset release
...
print(f"Loaded test dl for subj{test_subj} for evaluation!\n")
...
all_gt = f['images']
print("Loaded all 73k possible NSD images to cpu!", all_gt.shape)

# Initialize CLIP embedder
clip_embedder = CLIPImageEncoder('BigG', is_proj=True, return_type='pooled', process_img=True)
clip_seq_dim = 1
clip_embedder = clip_embedder.to(device)

if is_image and clip_embedder.__class__.__name__ == 'FrozenOpenCLIPImageEmbedder':
    # use tokens without projection
    clip_emb_dim = 1664 if 'BigG' in model_name else 1024
else:
    clip_emb_dim = 1280 if 'BigG' in model_name else 768

# Initialize model (uncomment desired model type)
...
# model = BrainMoE(num_voxels_list, hidden_dim, blurry_recon, n_blocks, clip_emb_dim, clip_seq_dim, clip_scale, interm_out=False, enc_version='v1', num_exp=16, capacity_factor=1)
model = BrainMoEMulti(num_voxels_list, hidden_dim, blurry_recon, n_blocks, clip_emb_dim, clip_seq_dim, clip_scale, interm_out=False, enc_version='v1', num_exp_0=2, capacity_factor_0=1, num_exp_layer=3, exp_factor_list=[2, 2, 2], cap_fac_list=[1, 1, 1], b_size=b_size)
# model = BrainMoEHier(num_voxels_list, hidden_dim, blurry_recon, n_blocks, clip_emb_dim, clip_seq_dim, clip_scale, interm_out=False, enc_version='v1', num_exps=[4, 8, 16, 32], capacity_factors=[1, 1, 1, 1], meta=True)
...
if train_router_only:
    assert ckpt_path, "Need to supply ckpt_path to train router only!"
...
def save_ckpt(tag):
    ...
    if accelerator.is_main_process:
        ...
        if use_prior: saving_dict.update({'prior_state_dict': accelerator.unwrap_model(diffusion_prior).state_dict()})
...
if local_rank==0 and wandb_log: # only use main process for wandb logging
    import wandb
    wandb_project = 'brainGen_Image_finetune'
...
for s, train_dl in enumerate(train_dls):
    with accelerator.autocast():
        ...
        for behav0, past_behav0, future_behav0, old_behav0 in train_dl:
            ...
            gt_iters[iter,s*batch_size:s*batch_size+batch_size] = image0
...
for epoch in progress_bar:
    ...
    for train_i in range(num_iterations_per_epoch):
        with accelerator.autocast():
            ...
            if use_prior:
                if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                    loss_prior = 0.
                    prior_out = []
                    for i in range(len(clip_voxels)):
                        l, o = diffusion_prior(text_embed=backbones[i], image_embed=clip_target)
                        loss_prior += l
                        prior_out.append(o)
            ...
            if clip_scale > 0:
                if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                    clip_voxels_norm = [nn.functional.normalize(x.flatten(1), dim=-1) for x in clip_voxels]
                else:
                ...
                if epoch < int(mixup_pct * num_epochs):
                    if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                        loss_clip = 0.
                        for i in range(len(clip_voxels)):
                            loss_clip += utils.mixco_nce(clip_voxels_norm[i], clip_target_norm,
                                temp=.006 * temp_coeff, perm=perm, betas=betas, select=select)
                        loss_clip /= len(clip_voxels)
                    else:
                    ...
                    if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                        loss_clip = 0.
                        for i in range(len(clip_voxels)):
                            loss_clip += utils.soft_clip_loss(clip_voxels_norm[i], clip_target_norm, temp=epoch_temp)
                        loss_clip /= len(clip_voxels)
                    else:
            ...
            labels = torch.arange(len(clip_target_norm)).to(clip_target_norm.device)  # Batch size!!!
            if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                fwd_percent_correct += np.mean([utils.topk(utils.batchwise_cosine_similarity(clip_voxels_norm[i], clip_target_norm), labels, k=1).item() for i in range(len(clip_voxels_norm))])
                bwd_percent_correct += np.mean([utils.topk(utils.batchwise_cosine_similarity(clip_target_norm, clip_voxels_norm[i]), labels, k=1).item() for i in range(len(clip_voxels_norm))])
                fwd_percent_correct_prior += np.mean([utils.topk(utils.batchwise_cosine_similarity(prior_out[i].flatten(-2), clip_target.flatten(-2)), labels, k=1).item() for i in range(len(clip_voxels_norm))])
                bwd_percent_correct_prior += np.mean([utils.topk(utils.batchwise_cosine_similarity(clip_target.flatten(-2), prior_out[i].flatten(-2)), labels, k=1).item() for i in range(len(clip_voxels_norm))])
            else:
            ...
            if mse_scale > 0:
                # loss_mse = mse(clip_voxels_norm, clip_target_norm)
                loss_mse = mse(clip_voxels.flatten(1), clip_target.flatten(1))
                loss += (loss_mse * mse_scale)

            if cos_scale > 0:
                if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                    clip_voxels_norm = [F.normalize(x.flatten(-2), dim=-1) for x in prior_out]
                    loss_cos = sum([1 - F.cosine_similarity(clip_voxels_norm[i], clip_target_norm).mean() for i in range(len(clip_voxels_norm))])
                    loss_cos /= len(clip_voxels_norm)
                else:
            ...
            if prior_clip_scale > 0:
                if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                    prior_norm = [F.normalize(prior_out[i].flatten(-2), dim=-1) for i in range(len(prior_out))]
                else:
                    prior_norm = F.normalize(prior_out.flatten(-2), dim=-1)
                if epoch < int(mixup_pct * num_epochs):
                    if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                        loss_clip_prior = 0.
                        for i in range(len(clip_voxels)):
                            loss_clip_prior += utils.mixco_nce(prior_norm[i], clip_target_norm,
                                temp=.006 * temp_coeff, perm=perm, betas=betas, select=select)
                        loss_clip_prior /= len(clip_voxels)
                    else:
                        loss_clip_prior = utils.mixco_nce(prior_norm, clip_target_norm, temp=.006 * temp_coeff, perm=perm, betas=betas, select=select)
                else:
                    epoch_temp = soft_loss_temps[epoch-int(mixup_pct*num_epochs)]
                    if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                        loss_clip_prior = 0.
                        for i in range(len(clip_voxels)):
                            loss_clip_prior += utils.soft_clip_loss(prior_norm[i], clip_target_norm, temp=epoch_temp)
                        loss_clip_prior /= len(clip_voxels)
                    else:
                        loss_clip_prior = utils.soft_clip_loss(prior_norm, clip_target_norm, temp=epoch_temp)
                loss += loss_clip_prior * 1
            ### Prior CLIP loss ###


            if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:       
                recon_cossim += np.mean([nn.functional.cosine_similarity(prior_out[i], clip_target).mean().item() for i in range(len(prior_out))])
                recon_mse += np.mean([mse(prior_out[i], clip_target).item() for i in range(len(prior_out))])
            else:
            ...
            if blurry_recon:  # low-level submodule
                ...
                image_enc = autoenc.encode(2*gt-1).latent_dist.mode() * 0.18215  # upsampler
                ...
                image_norm = (gt - mean)/std
                image_aug = (blur_augs(gt) - mean)/std
                ...
                cont_loss = accelerator.gather(cont_loss.detach()).mean().item()
                loss_blurry_cont_total += cont_loss

                with torch.no_grad():
                    # only doing pixcorr eval on a subset of the samples per batch because its costly & slow to compute autoenc.decode()
                    random_samps = np.random.choice(np.arange(len(gt)), size=len(gt)//5, replace=False)
                    blurry_recon_images = (autoenc.decode(image_enc_pred[random_samps]/0.18215).sample/ 2 + 0.5).clamp(0,1)
                    pixcorr = utils.pixcorr(gt[random_samps], blurry_recon_images)

                    pixcorr = accelerator.gather(pixcorr.detach()).mean().item()
                    blurry_pixcorr += pixcorr
    ...
    recon_cossim = accelerator.gather(torch.tensor(recon_cossim, device=accelerator.device)).mean()
    recon_mse = accelerator.gather(torch.tensor(recon_mse, device=accelerator.device)).mean()
    ...
    with torch.no_grad(), accelerator.autocast():
        ...
        for test_gt_, test_voxel_ in zip(test_gts, test_voxels):
            for test_i, i in enumerate(range(0, len(test_gt_), TEST_BZ)):
                ...
                clip_target = clip_embedder(test_gt)
                ...
                for rep in range(3):
                    ...
                    if rep == 0:
                        ...
                        if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                            clip_voxels = [x + x0 for x, x0 in zip(clip_voxels, clip_voxels0)]
                            backbone = [x + x0 for x, x0 in zip(backbone, backbone0)]
                        else:
                            clip_voxels += clip_voxels0
                            backbone += backbone0

                if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                    clip_voxels = [x/3 for x in clip_voxels]
                    backbone = [x/3 for x in backbone]
                else:
                    clip_voxels /= 3
                    backbone /= 3
                ...
                if use_prior:
                    if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                        ...
                        prior_out = []
                        for i in range(len(clip_voxels)):
                            l, o = diffusion_prior(text_embed=backbone[i], image_embed=clip_target)
                            loss_prior += l
                            prior_out.append(o)
                        loss_prior /= len(clip_voxels)
                    else:
                    ...
                    if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                        test_recon_cossim += np.mean([nn.functional.cosine_similarity(prior_out[i], clip_target).mean().item() for i in range(len(prior_out))])
                        test_recon_mse += np.mean([mse(prior_out[i], clip_target).item() for i in range(len(prior_out))])
                    else:
                    ...
                    prior_out = clip_voxels
                    test_recon_cossim += nn.functional.cosine_similarity(clip_voxels, clip_target).mean().item()
                    test_recon_mse += mse(clip_voxels, clip_target).item()
                ...
                if clip_scale > 0:
                    ...
                    if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                        loss_clip = 0.
                        for i in range(len(clip_voxels)):
                            loss_clip += utils.soft_clip_loss(clip_voxels_norm[i], clip_target_norm, temp=.006 * temp_coeff)
                        loss_clip /= len(clip_voxels)
                    else:
                        loss_clip = utils.soft_clip_loss(clip_voxels_norm, clip_target_norm, temp=.006 * temp_coeff)
                    # for v, t in zip(clip_voxels_interm_norm, clip_target_interm_norm):
                    #     loss_clip += utils.mixco_nce(v, t, temp=.006)

                    loss += (loss_clip * clip_scale)
                    test_loss_clip_total += loss_clip.item()

                    # forward and backward top 1 accuracy        
                    labels = torch.arange(len(clip_target_norm)).to(clip_target_norm.device)
                    if isinstance(clip_voxels, list) or clip_voxels.ndim == 4:
                        test_fwd_percent_correct += np.mean([utils.topk(utils.batchwise_cosine_similarity(clip_voxels_norm[i], clip_target_norm), labels, k=1).item() for i in range(len(clip_voxels_norm))])
                        test_bwd_percent_correct += np.mean([utils.topk(utils.batchwise_cosine_similarity(clip_target_norm, clip_voxels_norm[i]), labels, k=1).item() for i in range(len(clip_voxels_norm))])
                        test_fwd_percent_correct_prior += np.mean([utils.topk(utils.batchwise_cosine_similarity(prior_out[i].flatten(-2), clip_target.flatten(-2)), labels, k=1).item() for i in range(len(clip_voxels_norm))])
                        test_bwd_percent_correct_prior += np.mean([utils.topk(utils.batchwise_cosine_similarity(clip_target.flatten(-2), prior_out[i].flatten(-2)), labels, k=1).item() for i in range(len(clip_voxels_norm))])
                    else:
                ...
                random_samps = np.random.choice(np.arange(len(test_gt)), size=len(test_gt)//5, replace=False)
                ...
                if blurry_recon:
                    ...
                    blurry_recon_images = (autoenc.decode(image_enc_pred[random_samps]/0.18215).sample / 2 + 0.5).clamp(0,1)
                    pixcorr = utils.pixcorr(test_gt[random_samps], blurry_recon_images)
        ...
        if local_rank == 0:
            ...
            logs = {"train/loss": np.mean(losses[-(train_i+1):]),
                ...
                "train/recon_cossim": float(recon_cossim) / (train_i + 1),
                "test/recon_cossim": test_recon_cossim / (test_i + 1),
                "train/recon_mse": float(recon_mse) / (train_i + 1),
    ...
    if local_rank == 0:
        ...
        if (test_bwd_percent_correct + test_fwd_percent_correct) / 2 > best_acc:
            best_acc = (test_bwd_percent_correct + test_fwd_percent_correct) / 2
